 ...
 * OpenRouter Provider.
 *
 * Integration with OpenRouter's unified API for accessing multiple AI models.
 *
 ...
 *
 * Implements the Provider_Interface for the OpenRouter API.
 * OpenRouter uses an OpenAI-compatible endpoint and provides access to models
 * from multiple providers (Anthropic, OpenAI, Google, Meta, etc.) through a
 * single API key.
 *
 * @since 1.0.0
 */
final class OpenRouter_Provider implements Provider_Interface {
	 ...
	 * @var string
	 */
	private const DEFAULT_BASE_URL = 'https://openrouter.ai';
	...
	public function test_connection(): array {
		if (! $this->is_configured()) {
			return [
				'success' => false,
				'message' => __('OpenRouter API key is not configured.', 'seo-ai'),
			];
		}
	...
	public function chat(string $system_prompt, string $user_prompt, array $options = []): string {
		...
		if (empty($api_key)) {
			throw new \RuntimeException(
				__('OpenRouter API key is not configured.', 'seo-ai')
			);
		}
		...
		if (is_wp_error($response)) {
			throw new \RuntimeException(
				sprintf(
					...
					__('OpenRouter API request failed: %s', 'seo-ai'),
		...
		if ($status_code < 200 || $status_code >= 300) {
			...
			throw new \RuntimeException(
				sprintf(
					...
					__('OpenRouter API error: %s', 'seo-ai'),
					$error_message
				)
			);
		}
		...
		if (! isset($data['choices'][0]['message']['content'])) {
			throw new \RuntimeException(
				__('OpenRouter API returned an unexpected response format.', 'seo-ai')
			);
		}
